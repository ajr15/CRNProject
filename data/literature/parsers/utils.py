...
class NasaPolynomial:
    ...
    def cp(self, T):
        ...
        return self.R * (a[0] + a[1]*T + a[2]*T**2 + a[3]*T**3 + a[4]*T**4)

    def h(self, T):
        """Enthalpy (J/mol)"""
        a = self._select_coeffs(T)
        return self.R * T * (a[0] + a[1]*T/2 + a[2]*T**2/3 + a[3]*T**3/4 + a[4]*T**4/5 + a[5]/T)

    def s(self, T):
        """Entropy (J/mol·K)"""
        a = self._select_coeffs(T)
        return self.R * (a[0]*np.log(T) + a[1]*T + a[2]*T**2/2 + a[3]*T**3/3 + a[4]*T**4/4 + a[6])
