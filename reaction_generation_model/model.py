...
import pandas as pd
from scipy.stats.distributions import rv_continuous, rv_discrete, binom
...
class SimulatedReactionGraph:
    ...
    def add_kinetic_data(self, temperature: float, log_difference_distribution: Optional[rv_continuous]=None, ey_property_name: str="k_ey", real_property_name: str="k", min_log_k=-30, max_log_k=30):
        ...
        # scaling relations from Konnov network
        a = -5.004202e4 / temperature + 7.701469
        b = -7.475153e3 / temperature + 3.051622e1
        for rxn in self.get_reactions():
            dE = self.get_properties(rxn)["energy"]
            log_fitted_k = max(min_log_k, min(max_log_k, a * dE + b))
            self.get_properties(rxn)[ey_property_name] = np.exp(log_fitted_k)
            if log_difference_distribution is not None:
                while True:
                    log_k = log_difference_distribution.rvs() + log_fitted_k
                    if min_log_k < log_k < max_log_k:
                        self.get_properties(rxn)[real_property_name] = np.exp(log_k)
                        break
...
class KineticSolver:
    ...
    def _build_target_f(self):
        ...
        for rxn in self.g.get_reactions():
            rids.append([int(s[1:]) - 1 for s in self.g.get_reactants(rxn)])
            pids.append([int(s[1:]) - 1 for s in self.g.get_products(rxn)])
    ...
    def get_jacobian(self, sp: str):
        ...
        for rxn in self.g.consuming_reactions(sp):
            ...
            for s in self.g.get_reactants(rxn):
                if s != sp:
                    sp_idxs[-1].append(int(s[1:]) - 1)
            if len(sp_idxs[-1]) == 0:
                sp_idxs[-1].append(int(sp[1:]) - 1)
    ...
    def get_concentration(self, sp: str, step: int) -> float:
        return self._concs[step][int(sp[1:])]
...
if __name__ == "__main__":
    ...
    temperature = 1000
    model = PreferentialAttachmentModel(n_species=30, 
                                        n_reactions=1000, 
                                        specie_energy_dist=norm(0.4, 0.3))
    g = model.generate_graph()
    log_difference_distribution = norm(0, 1.105686e4 / temperature - 7.943808e-1) # fitted from konnov data
